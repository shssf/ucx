...
UCS_TEST_P(test_uct_stats, am_zcopy)
{
    ...
    uct_iov_t iov[1];
    iov[0].buffer = lbuf->ptr();
    iov[0].length = lbuf->length();
    iov[0].memh   = lbuf->memh();

    status = uct_ep_am_zcopy(sender_ep(), 0, 0, 0, iov, 1, NULL);
...
UCS_TEST_P(test_uct_stats, put_zcopy)
{
    ...
    status = uct_ep_put_zcopy(sender_ep(), iov, 1, rbuf->addr(), rbuf->rkey(), 0);
    EXPECT_TRUE(UCS_INPROGRESS == status || UCS_OK == status);
...
UCS_TEST_P(test_uct_stats, get_zcopy)
{
    ...
    status = uct_ep_get_zcopy(sender_ep(), iov, 1, rbuf->addr(), rbuf->rkey(), 0);
    EXPECT_TRUE(UCS_INPROGRESS == status || UCS_OK == status);
