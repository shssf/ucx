...
UCS_TEST_P(test_uct_ep, disconnect_after_send) {
    ...
    uct_iov_t iov[1];
    iov[0].buffer = buffer.ptr();
    iov[0].length = ucs_min(buffer.length(), m_sender->iface_attr().cap.am.max_zcopy);
    iov[0].memh   = buffer.memh();

    for (int i = 0; i < 100 / ucs::test_time_multiplier(); ++i) {
        connect();
        count = 0;
        for (;;) {
            status = uct_ep_am_zcopy(m_sender->ep(0), 1, NULL, 0, iov, 1, NULL);
